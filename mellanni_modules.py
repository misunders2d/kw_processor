...
def asin_alerts(market = 'US'):
    import keepa
    from datetime import datetime as dt
    import time
    import threading
    import numpy as np
    import telebot #if doesn't exist use os.system('pip install pytelegrambotapi')
    from telebot import types
    tg_token = '5465813080:AAEMK3qUadu127HmyM1JvVCi-C4GygCfIV0'
    keepa_key = '4l9vi7aaog62c5rnlgoml1ft7d2evd626nqfb6e6hel8f4akhkhrk2keru0funfi'
    top_sellers = {
        'B00NLLUMOE':'BedSheetSet-Queen-White-FBA','B00NQDGAP2':'BedSheetSet-King-White-FBA',
        'B00SBZJ8NG':'BedSheetSet-Queen-Light-Gray-FBA','B00NLLUP4G':'BedSheetSet-Queen-Gray-FBA',
        'B00SU0QSZ8':'BedSheetSet-King-Light-Gray-FBA','B00NQDGCW8':'BedSheetSet-King-Gray-FBA',
        'B00O35DAL4':'BedSheetSet-Full-White-FBA','B00O35CWQ8':'BedSheetSet-Twin-White-FBA',
        'B0822X1VP7':'M-21-BED-SHEET-SET-K-WHITE','B0822X4TLW':'M-21-BED-SHEET-SET-Q-WHITE'}
    test_sellers = {'B01E7UJBYY':'M-Quatrefoil-SheetSet-CK-Navy-Blue-FBA',
                    'B01E7UJCKM':'M-Quatrefoil-SheetSet-TXL-Navy-Blue-FBA'}
    sellers = top_sellers
    
    asins = [x for x in sellers.keys()]
    global result
    result = None
    ...
    def price_check():
        products = api.query(asins, domain = market)
        asin_list = []
        prices = []
        for l in range(len(products)):
            asin_list.append(products[l]['asin'])
            prices.append(products[l]['data']['df_NEW'].values[-1][0])
        result = list(zip(asin_list, prices))
        return result
    ...
    def alerts_check():
        ...
        for product in result:
            if np.isnan(product[1]):
                alerts.append(f'Alert! ASIN {product[0]} has lost the buybox!\nSKU: {sellers[product[0]]}')
        return alerts, result
    @bot.message_handler(commands=["start"])
    def start(m, res=False):
            markup=types.ReplyKeyboardMarkup(resize_keyboard=False)
            item1=types.KeyboardButton("go")
            item2=types.KeyboardButton("halt")
            item3=types.KeyboardButton("Check status")
            markup.add(item1)
            markup.add(item2)
            markup.add(item3)
            bot.send_message(m.chat.id,'ready, type /help for help',reply_markup =markup)

    def interval_check():
        global time_str, result
        while working:
            try:
                alerts, result = alerts_check()
                # alerts = []
                # result = [('0','1'),('0','1')]
                if len(alerts)>0:
                    bot.send_message('-1001793816204','\n'.join(alerts))
                else:
                    pass
                    # bot.send_message('-1001793816204','no new alerts')
                time_str = dt(*time.gmtime()[0:6]).strftime('%Y-%m-%d %H:%M:%S GMT')
            except:
                time_str = dt(*time.gmtime()[0:6]).strftime('%Y-%m-%d %H:%M:%S GMT: NO SUCCESS')
                bot.send_message('330959414',f'Price check failed on {time_str}')
            finally:
                print(f"Last check:\n{time_str}")
                time.sleep(30*60)
        return None            
    ...
    def handle_text(message):
        global working
        process = threading.Thread(target = interval_check)
        if message.text.strip() == 'Check status':
            # print(message.chat.id)
            if result is not None:
                bot.send_message(message.chat.id, '\n'.join([': '.join([x[0],str(x[1])]) for x in result]))
                bot.send_message(message.chat.id, f'updated on {time_str}')
            else:
                bot.send_message(message.chat.id, 'No results yet')
        elif message.text.strip() == 'go':
            working = True
            if process.is_alive() == False:
                process = threading.Thread(target = interval_check)
                bot.send_message(message.chat.id,'start pressed')
                process.start()
            else:
                bot.send_message(message.chat.id,'process already started')
        elif message.text.strip() == 'halt':
            bot.send_message(message.chat.id,'stop pressed')
            working = False
            if process.is_alive():
                process.join()
        elif message.text.strip() == '/help':
            help_message = '''type /drop to remove an ASIN\n
            type /add to add an ASIN
            '''
